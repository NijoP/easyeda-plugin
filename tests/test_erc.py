...
def test_floating_pin_is_error():
    ...
    assert any(x["rule"] == "floating_pin" and x["severity"] == "error" and x["where"] == "R1.2"
               for x in v), v
...
def test_no_ground_is_error():
    ...
    v = erc.run_erc(Enet.from_dict(d))
    assert any(x["rule"] == "no_ground" for x in v), v
...
def test_single_pin_net_dangling():
    ...
    v = erc.run_erc(Enet.from_dict(d))
    assert any(x["rule"] == "single_pin_net" and x["where"] == "SDA" for x in v), v
...
def test_power_rail_without_decoupling_warns():
    ...
    v = erc.run_erc(Enet.from_dict(d))
    w = [x for x in v if x["rule"] == "power_no_decoupling"]
